...
abstract class GenericLogger {
  ...
  public Class<?> simplifyPrimTypes(Class<?> type) {
  ...
  public <T> void addSupplier(String key, Class<?> type, Supplier<?> supplier, LogLevel level, boolean once) {
    type = simplifyPrimTypes(type);
    if (type.isArray()) {
      // Array types
      Class<?> componentType = simplifyPrimTypes(type.getComponentType());
      ...
      } else if (componentType.equals(Quantity.class)) {
        if (once) {
          put(key, Arrays.stream(((Quantity<?>[]) supplier.get())).mapToDouble(Quantity::getSiValue).toArray(), level);
        } else {
          addDoubleArray(key, () -> Arrays.stream(((Quantity<?>[]) supplier.get())).mapToDouble(Quantity::getSiValue).toArray(), level);
        }
